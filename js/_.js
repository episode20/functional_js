...
function _filter(list, predi){
  ...
  _each(list, function(val){
    if(predi(val)) new_lists.push(val);
  });

  // for(var i=0;i<list.length;i++){
  //   if( predi(list[i]) ){
  //     new_lists.push(list[i]);
  //   }
  // }
  return new_lists;
}
...
// object에서 value를 꺼낼때는 의미가 있음.
// function _values(data){
//   return _map(data, _identity)
// }
var _values = _map(_identity);

// identidy 함수를 합성할 수 있음
...
// some
// _some([1,2,4,10,30], function(val){ return val%10;})
// some => 조건에 하나라도 있다면 true, 아니면 false
// every => 조건에 모두 맞다면 true, 아니면 false
